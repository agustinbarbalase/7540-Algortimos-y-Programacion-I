...
void inicializar_nivel(nivel_t* nivel, int numero_nivel, coordenada_t personaje, coordenada_t sombra, config_t configuraciones[MAX_CONFIG]) {
	...
	agregar_elementos(nivel, VELAS, configuraciones[numero_nivel - 1].velas, personaje, sombra, 
		(*nivel).obstaculos, &((*nivel).tope_obstaculos)
	);
	agregar_elementos(nivel, POZOS, configuraciones[numero_nivel - 1].pozos, personaje, sombra, 
		(*nivel).obstaculos, &((*nivel).tope_obstaculos)
	);
	agregar_elementos(nivel, INTERRUPTORES, configuraciones[numero_nivel - 1].interruptores, personaje, sombra, 
		(*nivel).obstaculos, &((*nivel).tope_obstaculos)
	);
	agregar_elementos(nivel, PORTALES, configuraciones[numero_nivel - 1].portales, personaje, sombra, 
		(*nivel).obstaculos, &((*nivel).tope_obstaculos)
	);

	// Agregamos herramientas
	agregar_elementos(
		nivel, MONEDAS, configuraciones[numero_nivel - 1].monedas, personaje, sombra, 
		(*nivel).herramientas, &((*nivel).tope_herramientas)
	);
	agregar_elementos(
		nivel, LLAVE, configuraciones[numero_nivel - 1].llaves, personaje, sombra, 
		(*nivel).herramientas, &((*nivel).tope_herramientas)
	);
	inicializar_escaleras(nivel, configuraciones[numero_nivel - 1].escaleras, personaje, sombra);
